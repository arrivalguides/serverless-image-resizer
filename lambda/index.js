  ...
  if(ALLOWED_DIMENSIONS.size > 0 && !ALLOWED_DIMENSIONS.has(dimensions)) {
    ...
    S3.getObject(params).promise().then(data => {
        ...
        if ((originalExtension !== 'webp') && (event.headers !== undefined) && ('accept' in event.headers) && ( event.headers['accept'].indexOf('webp') > -1)) {
                ...
                return image.resize(width, height).sharpen().quality(parseInt(IMAGE_QUALITY)).toFormat('webp').toBuffer();
        } 
            
        return image.resize(width, height).sharpen().quality(parseInt(IMAGE_QUALITY)).toBuffer();
